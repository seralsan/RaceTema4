...
```{r}
cran %>% filter(size > 100500, r_arch == "linux-gnu")
```
...
```{r}
avg_bytes <- cran %>% summarise(avg_size = mean(size, na.rm = TRUE))
avg_bytes
```
# Agrupamientos
...
pack_sum <- by_package %>%
  summarise(
    count = n(),  # Número total de descargas
    unique = n_distinct(ip_id),  # Número de ordenadores únicos
    countries = n_distinct(country),  # Número de países distintos
    avg_bytes = mean(size, na.rm = TRUE),  # Tamaño medio de descarga
    med_bytes = median(size, na.rm = TRUE)  # Mediana del tamaño de descarga
  )
...
top_unique_sorted <- cran %>%
  count(package, ip_id, name = "unique") %>% 
  count(package, wt = unique) %>% 
  filter(n > quantile(n, probs = 0.99)) %>%
  arrange(desc(n))
...
# Cargar el data frame students
load("./data/datostidyr.Rdata")

# Transformar a formato tidy
...
students2_tidy <- students2 %>%
  pivot_longer(cols = -grade, names_to = c("sex", "class"), names_sep = "_", values_to = "count")
...
students3_tidy <- students3 %>%
  pivot_longer(cols = starts_with("class"), names_to = "class", values_to = "score", values_drop_na = TRUE) %>%
  separate(class, into = c("class", "exam"), sep = "_") %>%
